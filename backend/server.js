...
// Helper functions for Standards logic
const getCollection = async (name, data) => data;
const insertOne = async (name, item, collection) => { collection.push(item); return item; };
const updateOne = async (name, query, update, collection) => {
  const item = collection.find(i => i._id === query._id);
  if (item) Object.assign(item, update);
  return item;
};
const deleteOne = async (name, query, collection) => {
  const index = collection.findIndex(i => i._id === query._id);
